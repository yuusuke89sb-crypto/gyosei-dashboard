...
const OssDocuWorks = {
  ...
  async generatePdfBlob(payload) {
    ...
    const sozaiPngSrc = payload.sozaiMapPng || payload.mapPng;
    ...
    const cellW = 19.515;
    const startX = 225.7;
    const digitY = 595 - 67.0; // 528.0 pt

    // Draw 8 digits on Sozai Page & Haichi Page into cells 8..15
    for (let i = 0; i < 8; i++) {
      const ch = payload.orderNo8[i] || '0';
      const cx = startX + i * cellW + 5.0;
      sozaiPage.drawText(ch, { x: cx, y: digitY, size: 16.5, font, color: rgb(0, 0, 0) });
      haichiPage.drawText(ch, { x: cx, y: digitY, size: 16.5, font, color: rgb(0, 0, 0) });
    }

    // Embed Dealer Info overlay (Dealer name + Phone)
    try {
    ...
    // Embed Registration Number overlay (RegNo / 増車)
    if (payload.regNo) {
      try {
