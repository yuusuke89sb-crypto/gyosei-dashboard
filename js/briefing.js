  ...
  sendToLine() {
    ...
    if (events.length > 0) {
      ...
      events.forEach(e => {
        ...
        const memoText = (e.memo && typeof e.memo === 'string' && e.memo.trim()) ? `\n   📝 ${String(e.memo).trim().replace(/\n/g, ' ')}` : '';
    ...
    if (cases.length === 0) {
      ...
      sortedLocations.forEach(locName => {
        ...
        actions.forEach(act => {
          ...
          const memoText = (act.memo && typeof act.memo === 'string' && act.memo.trim()) ? `\n※${String(act.memo).trim().replace(/\n/g, ' ')}` : '';
