...
// ─── スタンプ管理（削除・追加・初期化） ───
const DEFAULT_STAMPS = ['⬅ 申請地', '自　宅', '保管場所', '駐車場', '5m道路', '出入口', 'ガレージ', '来客用', '軽自動車用', '県道186号', '私道', 'ブロック塀', '木曽川環境クリーン', '卍 福昌寺', 'JA', '鉄工所'];

function loadStamps() {
  try {
    const saved = localStorage.getItem('syako_map_maker_stamps_v2');
    if (saved) {
...
function editSelectedText() {
  ...
  if (newText !== null) {
    pushHistory();
    if (item.type === 'text' || item.type === 'line-dim' || item.type === 'dist-line') {
      item.text = newText;
